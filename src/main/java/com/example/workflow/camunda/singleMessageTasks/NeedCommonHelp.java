...
@Service
public class NeedCommonHelp implements SingleMessageTask {
    @Autowired
    ...
    public void process(User user) throws Exception {
        SendQuickReplyMessageDto needHelpMessage = new SendQuickReplyMessageDto();
        needHelpMessage.setReceiverContactNumber(user.getPhoneNumber());
        needHelpMessage.setType(MESSAGE_TYPE_QUICK_REPLY);
        ...
        needHelpMessage.setQuickReplyMessage(messageService.generateQuickReplyMessage(
                new MessageContent(
                        templateService.format(MessageTemplate.RIDE_BOOKING_TYPE_HEADER, user.getPreferredLanguage()),
                        templateService.format(MessageTemplate.NEED_HELP, user.getPreferredLanguage())
                ),
                options,
                UUID.randomUUID().toString())
        );

        messageService.sendQuickReplyMessage(needHelpMessage);
    }
}
